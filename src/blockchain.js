    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if ((currentTime - messageTime) > 300) {
                reject(Error('Time elapsed greater than 5 minutes'));
            } else {
                if (bitcoinMessage.verify(message, address, signature)) {
                    var block = await this._addBlock(new BlockClass.Block({address:address, star:star}));
                    resolve(block)
                } else (reject(Error("Couldn't verify message")))
            }
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            self.chain.forEach(block => {
                ...
                if (block.height > 0 && block.previousBlockHash !== self.chain[block.height-1].hash) {
                    errorLog.push('Block #'+block.height+' previous hash not matching '+block.previousBlockHash+' /= '+self.chain[block.height-1].hash)
                }
            })
            Promise.all(promises).then((results) => {
                ...
                results.forEach(valid => {
                    if (!valid) {
                        errorLog.push('Block #'+blockIndex+' has invalid hash '+self.chain[blockIndex].hash);
                    }
                    blockIndex++;
                })
                resolve(errorLog);
